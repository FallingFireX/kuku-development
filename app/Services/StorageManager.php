    ...
    */
    ...
    public function depositStack($user, $stacks, $quantities) {
        ...
        try {
            foreach ($stacks as $key=>$stack) {
                ...
                if (!$storage) {
                    throw new \Exception('An invalid object was selected.');
                }
                ...
                if ($this->depositOrigins($user, $stack, $quantity, $storage)) {
                    ...
                    throw new \Exception('Unable to deposit object.');
                }
            }
    ...
    public function withdrawStack($user, $data) {
        ...
        try {
            ...
            if (isset($data['remove_one'])) {
                ...
                if (!$stack) {
                    throw new \Exception('Invalid storage object.');
                }
            ...
            if (!$stacks->count()) {
                throw new \Exception('No stacks selected.');
            }
            ...
            foreach ($stacks as $key=>$stack) {
                ...
                if ($this->withdrawDestination($user, $stack, $quantity)) {
                    ...
                    if ($stack->count < 0) {
                        throw new \Exception('Cannot withdraw more objects than were stored.');
                    }
                    $stack->save();
                    if ($stack->count == 0) {
                        $stack->delete();
                    ...
                    throw new \Exception('Unable to withdraw object.');
                }
            }
    ...
    public function withdrawDestination($user, $stack, $quantity) {
        ...
        switch ($stack->storer_type) {
            default: case 'App/Models/User/UserItem':       if (!(new InventoryManager)->creditItem($user, $user, $type, $data, $stack->storable, $quantity)) {
                return false;
            }
                // case 'App/Models/User/UserCurrency':        (new InventoryManager)->debitCurrency($user, null, $type, $data, $stack->currency_id, $quantity);
        }
